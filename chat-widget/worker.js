...
// Accepts {messages} or {message,history}. Always returns OpenAI-style {choices[0].message.content}

const GUARDRAILS = `
You are Tony speaking in FIRST PERSON. Always use "I" and "my".
Never call yourself a copilot/assistant/AI/bot. You are Tony.
Introduce yourself only on the FIRST turn of a NEW session; don't repeat it unless asked "who are you" or "what is this?".
  ...
  async fetch(req, env) {
    ...
    if (url.pathname === "/chat" && req.method === "POST") {
      try {
        ...
        if (!hasOpenAI && !hasGroq) return json({ error: "No provider key set (OPENAI_API_KEY or GROQ_API_KEY)." }, 500);
        ...
        if (hasOpenAI) {
          ...
          payload = { model: "llama-3.1-8b-instant", messages, temperature, max_tokens: 400 };
        }

        const upstream = await fetch(apiUrl, { method: "POST", headers, body: JSON.stringify(payload) });
        if (!upstream.ok) {
          const txt = await upstream.text();
          // Return OpenAI-style error-ish payload so client won't crash
          return json({
            choices: [{ message: { role: "assistant", content: "Upstream model error. Please try again." } }],
        ...
        // Ensure OpenAI-style response (some SDKs differ slightly)
        ...
        return json({
          choices: [{ message: { role: "assistant", content: "Server exception. Please try again." } }],
